/**
 * Akne: dermatolog vs kosmetolog maqolasini API ga sync qiladi.
 * Usage: npx tsx scripts/seedAcneDermatologistArticleToApi.ts
 */
...
function buildSeedArticle() {
  const staticArticle = ACNE_DERMATOLOGIST_VS_COSMETOLOGIST_ARTICLE;
...
async function main() {
  ...
  const match =
    bySlug.get(normalizeKey(ACNE_DERMATOLOGIST_VS_COSMETOLOGIST_ARTICLE.slug)) ??
    byTitle.get(normalizeKey(ACNE_DERMATOLOGIST_VS_COSMETOLOGIST_ARTICLE.title.uz)) ??
    null;
  ...
  if (match) {
    ...
    console.log('Updated:', ACNE_DERMATOLOGIST_VS_COSMETOLOGIST_ARTICLE.title.uz);
  } else {
    await withRetry('create', () => createArticle(payload, null, token));
    console.log('Created:', ACNE_DERMATOLOGIST_VS_COSMETOLOGIST_ARTICLE.title.uz);
  }
}
