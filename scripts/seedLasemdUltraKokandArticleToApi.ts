/**
 * LaseMD Ultra Qo'qon maqolasini API ga sync qiladi.
 * Usage: npx tsx scripts/seedLasemdUltraKokandArticleToApi.ts
 */
...
import { LASEMD_ULTRA_KOKAND_ARTICLE } from '../src/data/articles/lasemdUltraKokandArticle';
...
function buildSeedArticle() {
  const staticArticle = LASEMD_ULTRA_KOKAND_ARTICLE;
...
async function main() {
  ...
  const match =
    bySlug.get(normalizeKey(LASEMD_ULTRA_KOKAND_ARTICLE.slug)) ??
    byTitle.get(normalizeKey(LASEMD_ULTRA_KOKAND_ARTICLE.title.uz)) ??
    null;

  const payload = mapArticleToCreatePayload(article);
  const coverFile = loadPublicImage(LASEMD_ULTRA_KOKAND_ARTICLE.image);

  if (match) {
    await withRetry('update', () => updateArticle(match.id, payload, coverFile, token));
    console.log('Updated:', LASEMD_ULTRA_KOKAND_ARTICLE.title.uz);
  } else {
    await withRetry('create', () => createArticle(payload, coverFile, token));
    console.log('Created:', LASEMD_ULTRA_KOKAND_ARTICLE.title.uz);
  }
}
