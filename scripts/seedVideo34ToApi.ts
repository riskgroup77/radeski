/**
 * clinic-video-34 ni API ga yuklaydi.
 * Usage: npx tsx scripts/seedVideo34ToApi.ts
 */
...
const TARGET = CLINIC_VIDEOS.filter((v) => v.id === 'clinic-video-34');
...
async function main() {
  ...
  if (TARGET.length !== 1) throw new Error('clinic-video-34 missing in CLINIC_VIDEOS');
  ...
  const match =
    ...
    bySortOrder.get(video.sortOrder ?? 34);
