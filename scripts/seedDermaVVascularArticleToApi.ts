/**
 * Derma V tomir to'ri maqolasini API ga sync qiladi.
 * Usage: npx tsx scripts/seedDermaVVascularArticleToApi.ts
 */
...
import { DERMA_V_VASCULAR_ARTICLE } from '../src/data/articles/dermaVVascularArticle';
...
function buildSeedArticle() {
  const staticArticle = DERMA_V_VASCULAR_ARTICLE;
...
async function main() {
  ...
  const match =
    bySlug.get(normalizeKey(DERMA_V_VASCULAR_ARTICLE.slug)) ??
    byTitle.get(normalizeKey(DERMA_V_VASCULAR_ARTICLE.title.uz)) ??
    null;
  ...
  if (match) {
    ...
    console.log('Updated:', DERMA_V_VASCULAR_ARTICLE.title.uz);
  } else {
    await withRetry('create', () => createArticle(payload, null, token));
    console.log('Created:', DERMA_V_VASCULAR_ARTICLE.title.uz);
  }
}
