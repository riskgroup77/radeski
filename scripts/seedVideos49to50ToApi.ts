/**
 * clinic-video-49 … 50 ni API ga yuklaydi.
 * Usage: npx tsx scripts/seedVideos49to50ToApi.ts
 */
...
const TARGET = CLINIC_VIDEOS.filter((v) => {
  const n = Number(String(v.id).replace('clinic-video-', ''));
  return n === 49 || n === 50;
});
...
async function main() {
  if (TARGET.length !== 2) throw new Error(`Expected 2 videos, got ${TARGET.length}`);
