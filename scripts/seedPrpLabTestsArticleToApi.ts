/**
 * Faqat yangi PRP tahlillar maqolasini API ga sync qiladi.
 * Usage: npx tsx scripts/seedPrpLabTestsArticleToApi.ts
 */
...
function buildSeedArticle() {
  const staticArticle = PRP_LAB_TESTS_ARTICLE;
...
async function main() {
  ...
  const match =
    bySlug.get(normalizeKey(PRP_LAB_TESTS_ARTICLE.slug)) ??
    byTitle.get(normalizeKey(PRP_LAB_TESTS_ARTICLE.title.uz)) ??
    null;
  ...
  if (match) {
    ...
    console.log('Updated:', PRP_LAB_TESTS_ARTICLE.title.uz);
  } else {
    await withRetry('create', () => createArticle(payload, null, token));
    console.log('Created:', PRP_LAB_TESTS_ARTICLE.title.uz);
  }
}
