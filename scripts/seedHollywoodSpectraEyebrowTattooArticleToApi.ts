/**
 * Qosh tatuaji Hollywood Spectra maqolasini API ga sync qiladi.
 * Usage: npx tsx scripts/seedHollywoodSpectraEyebrowTattooArticleToApi.ts
 */
...
import { HOLLYWOOD_SPECTRA_EYEBROW_TATTOO_ARTICLE } from '../src/data/articles/hollywoodSpectraEyebrowTattooArticle';
...
function buildSeedArticle() {
  const staticArticle = HOLLYWOOD_SPECTRA_EYEBROW_TATTOO_ARTICLE;
...
async function main() {
  ...
  const match =
    bySlug.get(normalizeKey(HOLLYWOOD_SPECTRA_EYEBROW_TATTOO_ARTICLE.slug)) ??
    byTitle.get(normalizeKey(HOLLYWOOD_SPECTRA_EYEBROW_TATTOO_ARTICLE.title.uz)) ??
    null;

  const payload = mapArticleToCreatePayload(article);
  const coverFile = loadPublicImage(HOLLYWOOD_SPECTRA_EYEBROW_TATTOO_ARTICLE.image);

  if (match) {
    await withRetry('update', () => updateArticle(match.id, payload, coverFile, token));
    console.log('Updated:', HOLLYWOOD_SPECTRA_EYEBROW_TATTOO_ARTICLE.title.uz);
  } else {
    await withRetry('create', () => createArticle(payload, coverFile, token));
    console.log('Created:', HOLLYWOOD_SPECTRA_EYEBROW_TATTOO_ARTICLE.title.uz);
  }
}
