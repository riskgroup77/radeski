/**
 * Bolalarda so‘gal (lazer) maqolasini API ga sync qiladi.
 * Usage: npx tsx scripts/seedPediatricWartsLaserArticleToApi.ts
 */
...
import { PEDIATRIC_WARTS_LASER_RADESKI_ARTICLE } from '../src/data/articles/pediatricWartsLaserRadeskiArticle';
...
function buildSeedArticle() {
  const staticArticle = PEDIATRIC_WARTS_LASER_RADESKI_ARTICLE;
...
async function main() {
  ...
  const match =
    bySlug.get(normalizeKey(PEDIATRIC_WARTS_LASER_RADESKI_ARTICLE.slug)) ??
    byTitle.get(normalizeKey(PEDIATRIC_WARTS_LASER_RADESKI_ARTICLE.title.uz)) ??
    null;
  ...
  if (match) {
    ...
    console.log('Updated:', PEDIATRIC_WARTS_LASER_RADESKI_ARTICLE.title.uz);
  } else {
    await withRetry('create', () => createArticle(payload, null, token));
    console.log('Created:', PEDIATRIC_WARTS_LASER_RADESKI_ARTICLE.title.uz);
  }
}
