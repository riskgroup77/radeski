/**
 * Jinsiy a'zo yassi hujayrali raki maqolasini API ga sync qiladi.
 * Usage: npx tsx scripts/seedPenileSccArticleToApi.ts
 */
...
import { PENILE_SCC_ARTICLE } from '../src/data/articles/penileSccArticle';
...
function buildSeedArticle() {
  const staticArticle = PENILE_SCC_ARTICLE;
...
async function main() {
  ...
  const match =
    bySlug.get(normalizeKey(PENILE_SCC_ARTICLE.slug)) ??
    byTitle.get(normalizeKey(PENILE_SCC_ARTICLE.title.uz)) ??
    null;
  ...
  if (match) {
    ...
    console.log('Updated:', PENILE_SCC_ARTICLE.title.uz);
  } else {
    await withRetry('create', () => createArticle(payload, null, token));
    console.log('Created:', PENILE_SCC_ARTICLE.title.uz);
  }
}
