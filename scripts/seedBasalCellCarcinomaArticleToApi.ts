/**
 * Bazalioma maqolasini API ga sync qiladi.
 * Usage: npx tsx scripts/seedBasalCellCarcinomaArticleToApi.ts
 */
...
function buildSeedArticle() {
  const staticArticle = BASAL_CELL_CARCINOMA_ARTICLE;
...
async function main() {
  ...
  const match =
    bySlug.get(normalizeKey(BASAL_CELL_CARCINOMA_ARTICLE.slug)) ??
    byTitle.get(normalizeKey(BASAL_CELL_CARCINOMA_ARTICLE.title.uz)) ??
    null;
  ...
  if (match) {
    ...
    console.log('Updated:', BASAL_CELL_CARCINOMA_ARTICLE.title.uz);
  } else {
    await withRetry('create', () => createArticle(payload, null, token));
    console.log('Created:', BASAL_CELL_CARCINOMA_ARTICLE.title.uz);
  }
}
