/**
 * Trixolog/trixoskopiya maqolasini API ga sync qiladi.
 * Usage: npx tsx scripts/seedTrichologistArticleToApi.ts
 */
...
function buildSeedArticle() {
  const staticArticle = TRICHOLOGIST_TRICHOSCOPY_ARTICLE;
...
async function main() {
  ...
  const match =
    bySlug.get(normalizeKey(TRICHOLOGIST_TRICHOSCOPY_ARTICLE.slug)) ??
    byTitle.get(normalizeKey(TRICHOLOGIST_TRICHOSCOPY_ARTICLE.title.uz)) ??
    null;
  ...
  if (match) {
    ...
    console.log('Updated:', TRICHOLOGIST_TRICHOSCOPY_ARTICLE.title.uz);
  } else {
    await withRetry('create', () => createArticle(payload, null, token));
    console.log('Created:', TRICHOLOGIST_TRICHOSCOPY_ARTICLE.title.uz);
  }
}
