/**
 * Rozatseya (Radeski) maqolasini API ga sync qiladi.
 * Usage: npx tsx scripts/seedRosaceaRadeskiArticleToApi.ts
 */
...
function buildSeedArticle() {
  const staticArticle = ROSACEA_RADESKI_ARTICLE;
...
async function main() {
  ...
  const match =
    bySlug.get(normalizeKey(ROSACEA_RADESKI_ARTICLE.slug)) ??
    byTitle.get(normalizeKey(ROSACEA_RADESKI_ARTICLE.title.uz)) ??
    null;
  ...
  if (match) {
    ...
    console.log('Updated:', ROSACEA_RADESKI_ARTICLE.title.uz);
  } else {
    await withRetry('create', () => createArticle(payload, null, token));
    console.log('Created:', ROSACEA_RADESKI_ARTICLE.title.uz);
  }
}
