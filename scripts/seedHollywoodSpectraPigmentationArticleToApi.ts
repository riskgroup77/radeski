/**
 * Hollywood Spectra pigmentatsiya maqolasini API ga sync qiladi.
 * Usage: npx tsx scripts/seedHollywoodSpectraPigmentationArticleToApi.ts
 */
...
import { HOLLYWOOD_SPECTRA_PIGMENTATION_ARTICLE } from '../src/data/articles/hollywoodSpectraPigmentationArticle';
...
function buildSeedArticle() {
  const staticArticle = HOLLYWOOD_SPECTRA_PIGMENTATION_ARTICLE;
...
async function main() {
  ...
  const match =
    bySlug.get(normalizeKey(HOLLYWOOD_SPECTRA_PIGMENTATION_ARTICLE.slug)) ??
    byTitle.get(normalizeKey(HOLLYWOOD_SPECTRA_PIGMENTATION_ARTICLE.title.uz)) ??
    null;
  ...
  if (match) {
    ...
    console.log('Updated:', HOLLYWOOD_SPECTRA_PIGMENTATION_ARTICLE.title.uz);
  } else {
    await withRetry('create', () => createArticle(payload, null, token));
    console.log('Created:', HOLLYWOOD_SPECTRA_PIGMENTATION_ARTICLE.title.uz);
  }
}
