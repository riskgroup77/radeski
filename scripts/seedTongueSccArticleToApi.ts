/**
 * Til SCC maqolasini API ga sync qiladi.
 * Usage: npx tsx scripts/seedTongueSccArticleToApi.ts
 */
...
function buildSeedArticle() {
  const staticArticle = TONGUE_SCC_ARTICLE;
...
async function main() {
  ...
  const match =
    bySlug.get(normalizeKey(TONGUE_SCC_ARTICLE.slug)) ??
    byTitle.get(normalizeKey(TONGUE_SCC_ARTICLE.title.uz)) ??
    null;
  ...
  if (match) {
    ...
    console.log('Updated:', TONGUE_SCC_ARTICLE.title.uz);
  } else {
    await withRetry('create', () => createArticle(payload, null, token));
    console.log('Created:', TONGUE_SCC_ARTICLE.title.uz);
  }
}
